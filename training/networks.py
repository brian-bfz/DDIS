...
# ----------------------------------------------------------------------------
# 3D convolutional layer with optional up/downsampling. Direct 3D analogue
# of Conv2d: separable resample filter, kernel=0 for pure resampling,
# fused_resample, and the same custom weight init.
...
class Conv3d(torch.nn.Module):
    def __init__(
        ...
        self.bias = torch.nn.Parameter(weight_init([out_channels], **init_kwargs) * init_bias) if kernel and bias else None
        f = torch.as_tensor(resample_filter, dtype=torch.float32)
        f = (f[:, None, None] * f[None, :, None] * f[None, None, :]).unsqueeze(0).unsqueeze(1) / f.sum().pow(3)
        self.register_buffer("resample_filter", f if up or down else None)
...
# ----------------------------------------------------------------------------
# 3D analogue of UNetBlock. Uses Conv3d in place of Conv2d; affine modulation
# broadcasts over (D, H, W); attention flattens all spatial dims into a single
# token sequence so AttentionOp is unchanged.
...
# ----------------------------------------------------------------------------
# 3D version of SongUNet. Operates on volumes (B, C, D, H, W) with cubic
# resolution D=H=W=img_resolution. Logic matches the 2D SongUNet exactly:
# same mapping, encoder/decoder topology, skip handling, and aux paths —
# only Conv2d → Conv3d and UNetBlock → UNet3DBlock are swapped, and module
# names use the "DxHxW" convention.
...
class SongUNet3D(torch.nn.Module):
    def __init__(
        self,
        img_resolution,  # Volume resolution at input/output (cubic: D=H=W=img_resolution).
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                self.enc[f"{res}x{res}x{res}_down"] = UNet3DBlock(in_channels=cout, out_channels=cout, down=True, **block_kwargs)
                if encoder_type == "skip":
            ...
            for idx in range(num_blocks):
                ...
                cout = model_channels * mult
                attn = res in attn_resolutions
                self.enc[f"{res}x{res}x{res}_block{idx}"] = UNet3DBlock(in_channels=cin, out_channels=cout, attention=attn, **block_kwargs)
        skips = [block.out_channels for name, block in self.enc.items() if "aux" not in name]
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            res = img_resolution >> level
            if level == len(channel_mult) - 1:
                self.dec[f"{res}x{res}x{res}_in0"] = UNet3DBlock(in_channels=cout, out_channels=cout, attention=True, **block_kwargs)
                self.dec[f"{res}x{res}x{res}_in1"] = UNet3DBlock(in_channels=cout, out_channels=cout, **block_kwargs)
            else:
                self.dec[f"{res}x{res}x{res}_up"] = UNet3DBlock(in_channels=cout, out_channels=cout, up=True, **block_kwargs)
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                attn = idx == num_blocks and res in attn_resolutions
                self.dec[f"{res}x{res}x{res}_block{idx}"] = UNet3DBlock(in_channels=cin, out_channels=cout, attention=attn, **block_kwargs)
            if decoder_type == "skip" or level == 0:
                if decoder_type == "skip" and level < len(channel_mult) - 1:
    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None):
        ...
        for name, block in self.enc.items():
            if "aux_down" in name:
                ...
                x = block(x, emb) if isinstance(block, UNet3DBlock) else block(x)
                skips.append(x)
